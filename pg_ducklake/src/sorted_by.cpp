...
sorted_ambuild(Relation heap, Relation index, IndexInfo *indexInfo) {
...
static void
sorted_ambuildempty(Relation index) {
}

static bool
sorted_aminsert(Relation rel, Datum *values, bool *isnull, ItemPointer ht_ctid, Relation heapRel,
                IndexUniqueCheck checkUnique, bool indexUnchanged, IndexInfo *indexInfo) {
	return false;
}

static IndexBulkDeleteResult *
sorted_ambulkdelete(IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback,
                    void *callback_state) {
	return stats;
}

static IndexBulkDeleteResult *
sorted_amvacuumcleanup(IndexVacuumInfo *info, IndexBulkDeleteResult *stats) {
	return stats;
}

static void
sorted_amcostestimate(PlannerInfo *root, IndexPath *path, double loop_count, Cost *indexStartupCost,
...
static bytea *
sorted_amoptions(Datum reloptions, bool validate) {
	return NULL;
}

static bool
sorted_amvalidate(Oid opclassoid) {
	return true;
}

static IndexScanDesc
sorted_ambeginscan(Relation rel, int nkeys, int norderbys) {
	return RelationGetIndexScan(rel, nkeys, norderbys);
}

static void
sorted_amrescan(IndexScanDesc scan, ScanKey keys, int nkeys, ScanKey orderbys, int norderbys) {
}

static void
sorted_amendscan(IndexScanDesc scan) {
}

Datum
ducklake_sorted_am_handler(PG_FUNCTION_ARGS) {
...
std::string
EscapeSQLString(const char *str) {
	std::string result("'");
...
std::string
Ruleutils::get_create_sorted_index_def(IndexStmt *stmt) {
	if (stmt->concurrent)
	...
	foreach (lc, stmt->indexParams) {
		...
		if (elem->name)
			...
			ereport(ERROR,
			        (errcode(ERRCODE_INVALID_OBJECT_DEFINITION), errmsg("index key must be a column or expression")));
...
void
ApplyCreateSortedIndex(const std::string &query) {
...
std::vector<SortedIndexDrop>
FindSortedIndexDrops(DropStmt *drop) {
...
void
CreateSortedIndexForTable(Oid relid, const char *sort_spec) {
	SyncSortedIndexes({{relid, sort_spec}}, {});
}

void
DropSortedIndexForTable(Oid relid) {
