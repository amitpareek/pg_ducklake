...
DECLARE_PG_FUNCTION(ducklake_create_table_trigger) {
	...
	if (!IsA(parsetree, CreateStmt) && !IsA(parsetree, CreateTableAsStmt)) {
		ereport(ERROR,
		        (errcode(ERRCODE_INVALID_TABLE_DEFINITION), errmsg("Cannot create a DuckLake table this way, use "
	...
	if (is_temporary) {
		ereport(ERROR,
		        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("temporary tables are not supported with ducklake "
		                                                        "access method")));
	}
...
static const char *
DucklakeTableAmHook(const TableAmRoutine *am) {
